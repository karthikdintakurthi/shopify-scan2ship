# Scan2Ship - Shopify Embedded App

A modern Shopify embedded app built with **Remix**, **TypeScript**, and **Polaris UI**. This app provides a streamlined interface for managing shipping operations within the Shopify Admin.
...
1. **Node.js** (v18.20+ or v20.10+): [Download and install](https://nodejs.org/en/download/)
...
4. **Shopify CLI**: Already installed globally via the setup process
...
2. **Configure your environment variables in `.env`:**
   ...
   # App URL (automatically set by Shopify CLI during development)
...
3. **Install on your development store:**
   - The CLI will provide a tunnel URL (ngrok)
   - Press `P` to open the app URL in your browser
   - Click "Install" to add the app to your development store
   - The app will be embedded in your Shopify Admin
...
- **Framework**: [Remix](https://remix.run) with TypeScript
- **UI Library**: [Shopify Polaris](https://polaris.shopify.com/)
- **Authentication**: [@shopify/shopify-app-remix](https://shopify.dev/docs/api/shopify-app-remix)
- **App Bridge**: [@shopify/app-bridge-react](https://shopify.dev/docs/api/app-bridge)
- **Database**: [Prisma](https://www.prisma.io/) with SQLite (development)
...
The app is configured as an embedded app with:
- **API Version**: 2025-10
- **Scopes**: `write_products`
- **Webhooks**: App uninstalled, scopes update
- **Distribution**: App Store ready

### Database

The app uses Prisma with SQLite for development. For production, consider:
- PostgreSQL
- MySQL
- MongoDB

Update `prisma/schema.prisma` and `DATABASE_URL` accordingly.
...
- **Embedded Experience**: Seamlessly integrated into Shopify Admin
- **Modern UI**: Built with Polaris design system
...
1. **Hot Reload**: Changes to your code will automatically reload
2. **Tunnel URL**: The CLI provides a secure tunnel for local development
3. **GraphQL**: Use the built-in GraphQL client for API calls
4. **Polaris Components**: Leverage the full Polaris component library
5. **TypeScript**: Enjoy full type safety and IntelliSense
